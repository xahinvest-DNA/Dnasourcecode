...
class OpenAIResponsesMeaningGenerator(MeaningGeneratorProtocol):
    ...
    def diagnosis(self, intake: dict[str, Any], dna: dict[str, Any]) -> dict[str, str]:
        ...
        system = (
            ...
            "Используй DNA support signals как скрытую диагностическую поддержку, но не как самостоятельный ответ. "
            ...
            "Особенно важно: если DNA указывает, что ценность сначала отдаётся бесплатно и только потом теряется момент платного перехода, "
